                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="patient_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->id }}">{{ $patient->first_name ?? $patient->name }} {{ $patient->last_name }} - {{ $patient->jmbg ?? '' }}</option>
                                    @endforeach
                                </select>
                                @error('patient_id')
                            ...
                            <div>
                                ...
                                <select name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    <option value="scheduled" {{ old('status') === 'scheduled' ? 'selected' : '' }}>Zakazan</option>
                                    <option value="cancelled" {{ old('status') === 'cancelled' ? 'selected' : '' }}>Otkazan</option>
                                    <option value="done" {{ old('status') === 'done' ? 'selected' : '' }}>Završen</option>
                                </select>
                                @error('status')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
